...
- 更新详情的「对照翻译」采用连续排版：每个段落或列表项的英文下紧跟中文，共用列表圆点/编号和缩进；标题保留原文，去掉卡片、底色和重复的语言标签。新结果保存显式对应关系；已有字符串译文全文对照，不通过数组下标猜配对。
...
- 递归发现引用定义或脚注时在存储层保留全文解析上下文。前端使用独立的原文/译文引用命名空间，在一份 Markdown 文档内交错排版，避免跨段链接失效。
...
根据用户参考图，展示层改为「英文条目 → 同缩进中文段落 → 下一条英文」，保留原文标题、编号、嵌套列表与链接。`src/shared/bilingual.ts` 仅对已有显式配对的 Markdown 块做结构检查与交错渲染；如果译文列表项数量或嵌套结构不同，保留整块对照，避免串项。历史无配对数据不自动重译，不产生新的模型费用。

验证：4 项 HTML 渲染测试覆盖同项中英顺序、链接、列表起始编号、嵌套列表、代码去重、引用隔离和结构不匹配；2 个浏览器场景检查桌面/手机的实际位置、缩进、条目数量与无卡片边框，截图人工检查通过。
